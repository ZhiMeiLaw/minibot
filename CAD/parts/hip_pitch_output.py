"""
Hip Pitch Output / Fork (V6-PRT-0011)
Mini-Atlas V6 Alpha
50 x 55 x 35 mm | fork type | shaft Ø8.10mm slip fit | clamp gap 2mm
...
doc = App.newDocument("HipPitchOutput")

W, D, H = P.HIP_PITCH_OUTPUT_W, P.HIP_PITCH_OUTPUT_D, P.HIP_PITCH_OUTPUT_H
gap = P.CLAMP_GAP
arm_t = 4.0
x_ctr = W / 2
z_ctr = H / 2
...
# Shaft hole - along Y axis (pitch axis)
...
boss_d = 22.0
boss_h = 5.0
for dy in [-P.BEARING_SPACING_HIP_PITCH/2, P.BEARING_SPACING_HIP_PITCH/2]:
...
# Brass insert holes for leg adapter connection
brass_ins_d = 4.4
pcd_leg = 30.0
...
for dx_sgn in [-1, 1]:
    dx = dx_sgn * 12
    rib = Part.makeBox(15, rib_t, H * 0.5,
                        App.Vector(x_ctr + dx - 7.5, -rib_t/2, H * 0.25))
    body.Shape = body.Shape.fuse(rib)

doc.recompute()
out = os.path.join(STL_DIR, "hip_pitch_output.stl")
mesh = Mesh.Mesh()
...
print(f"[OK] hip_pitch_output.stl -> {out}")